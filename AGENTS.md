...
7. **External Review Prompts & Ephemeral Living Review Branches**:
   ...
   - **Review Delivery Modes**:
     ...
     - **Mode B: Shared Sandbox Branch Mode**: When reviewers are pinned to a single shared branch (e.g., Arena.ai), reviewers MUST isolate their work into `reviews/<reviewer-id>.md`, adhere to `FILE ISOLATION` and `TARGETED STAGING` (`git add reviews/<reviewer-id>.md` only, no `git add .`), and push via bounded rebase-retry loops (`git pull --rebase origin <shared-branch>`). Force-pushing is strictly forbidden, and builder audits reject any `TAMPERED/CLOBBERED` commits.
