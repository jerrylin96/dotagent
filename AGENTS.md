...
7. **External Review Prompts & Ephemeral Living Review Branches**:
   ...
   - **Tamper Tripwire & Termination Directive**: Reviewers are strictly forbidden from modifying `reviewer_scorecard.md` or any file outside `reviews/${REVIEWER_ID}.md`. If builder authorship audit detects any unauthorized edit, the builder immediately notifies the user to terminate that reviewer's session, adds the agent to the Drop List, and rejects the commit.
