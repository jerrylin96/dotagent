...
7. **External Review Prompts & Ephemeral Living Review Branches**:
   ...
   - **In-Tree Ephemeral Prompts (`review_prompt.md`)**: External review prompts are committed in-tree to `${FEATURE_SLUG}/review_prompt.md` and dispatched via minimal 2-line chat pointers (`git fetch origin ${BRANCH_NAME} && cat ${FEATURE_SLUG}/review_prompt.md`), purged automatically with `${FEATURE_SLUG}/` at Step 7b.
