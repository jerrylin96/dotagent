...
- **Mode B: Shared Sandbox Branch Mode**: When all reviewers are pinned by platform to a single shared branch (e.g. Arena.ai):
  ...
  - **Builder Ingestion Authorship Audit & Universal Tamper Tripwire**:
    - When ingesting review branches, the builder verifies commit history scoped to shared branch commits (`git fetch origin <shared-branch> && git log --name-only "${before:-FETCH_HEAD}..FETCH_HEAD"`) and branch refs (with `before` tracked from the prior triaged SHA or defaulting to `FETCH_HEAD` on initial ingest to yield zero false positives).
